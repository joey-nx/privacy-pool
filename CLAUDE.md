...
- **다이어그램**: md파일일 경우 Mermaid만 사용, 터미널 환경 응답일 경우 ASCII
...
## 적용 시점
- 아키텍처 결정 (DB 선택, API 설계, 캐시 전략 등)
- 검토 요청 ("이 방법이 맞아?", "더 나은 방법 있어?")
- 버그 근본 원인 분석 — 패치 전에 구조적 원인 탐색
- 기술 비교 또는 논문·문서 리서치
...
**1단계 — 정(Thesis)**
관습적·표준적 해법을 먼저 서술한다.
암묵적으로 전제하고 있는 가정을 명시한다.
답이 명백해 보여도 이 단계를 건너뛰지 않는다.

**2단계 — 반(Antithesis)**
thesis가 깨지는 조건을 적극적으로 탐색한다:
- 에지 케이스 및 장애 시나리오
- 규모 확장 시 발생하는 한계
- 숨겨진 결합도 및 의존성 가정
- 부하 상황에서의 성능·일관성 트레이드오프
모순을 합리화하지 말고 반드시 표면화한다.

**3단계 — 합(Synthesis / Aufhebung)**
다음 조건을 만족하는 고차 해법을 제시한다:
- thesis의 강점을 보존
- antithesis의 모순을 억압하지 않고 해소
synthesis가 불가능한 경우, "미해결 트레이드오프" 섹션을
명시적으로 작성한다 — 억지 결론을 내리지 않는다.

## 기술 리서치 추가 규칙
- 논문·문서 요약: thesis = 저자 주장,
  antithesis = 반례·한계, synthesis = 실무 적용 가능성
- 기술 비교: thesis = 각 기술의 설계 의도,
  antithesis = 그 의도가 붕괴하는 맥락
- 벤치마크 데이터와 버전 정보는 반드시 인라인으로 출처를 표기

## 적용 범위
비자명한 아키텍처 결정에만 적용한다.
다음은 건너뛴다: 코드 포매팅, 네이밍, 단순 한 줄 수정.
